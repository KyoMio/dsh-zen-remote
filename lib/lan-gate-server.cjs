...
function servePwaAsset(req, res) {
  ...
  res.writeHead(200, { 'Content-Type': PWA_FILE_TYPES[ext] || 'application/octet-stream', 'Cache-Control': 'no-cache' })
  res.end(body)
  return true
}
...
// NOTE (S4.1, 2026-08-17): the fullscreen-dialog rules that used to live here
// -- `[role="dialog"][aria-modal="true"] { width:100vw; height:100dvh; ... }`
// in both the [data-lan-device="phone"] block and the (max-width:820px)
// fallback -- are GONE ON PURPOSE. Dialog layout on phones belongs to
// @dsh-external/dsh-mobile-nav (commit 3ec8f38 removed the same rules from
// pwa/app.css but missed this inline copy, so they kept shipping).
//
// They were not merely redundant, they were actively wrong: the selector
// matches ANY correctly-marked-up dialog, including dsh-mobile-nav's own
// session-info bottom sheet, which carries role="dialog" aria-modal="true"
// for accessibility. Through the gateway that sheet was forced to
// 100vw x 100dvh -- measured 390x844 at x:8,y:-8 in a 390x844 viewport,
// i.e. the whole card tiled fullscreen and spilling off two edges, versus a
// correct 374x367 at x:8,y:469 on bare :3080. Reproduced in BOTH WebKit and
// Chromium, so it was never an engine bug: the variable was gateway vs no
// gateway. dsh-mobile-nav guards its own copy of this generic selector with
// `:not([data-mobile-nav="info-sheet"])` (styles/layout.css.ts) -- the same
// hazard, already documented in its AGENTS.md.
//
// Do not reintroduce a blanket [aria-modal="true"] rule here. If the gateway
// ever needs one again, exclude dsh-mobile-nav's own sheets explicitly.
//
// The composer's home-indicator padding is gone from here too, for a separate
// reason: it targeted [data-slot="conversation.composer"], which is
// `display: contents` and so never took the padding at all. See pwa/app.css
// for the full note; the working rule now lives in dsh-mobile-nav's
// styles/composer.css.ts.
const DEVICE_CSS = '<style>html[data-lan-device="phone"]{--dsw-pwa-font-scale:1}html[data-lan-device="phone"] [data-slot="conversation"] [data-phase]{--dsw-font-s-14-font-size:12.5px;--dsw-font-xs-13-font-size:11.5px;--dsw-font-xxs-12-font-size:11px;--dsw-font-xxxs-11-font-size:10px;--dsw-font-markdown-base-font-size:14px;--dsw-font-markdown-small-font-size:13px;--dsw-font-markdown-code-font-size:12.5px;--dsw-font-markdown-code-block-font-size:12px;--dsw-font-markdown-h1-font-size:18px;--dsw-font-markdown-h2-font-size:16px;--dsw-font-markdown-h3-font-size:15px;--dsw-font-markdown-h4-font-size:14px}html[data-lan-device="phone"] [data-slot="conversation"] input,html[data-lan-device="phone"] [data-slot="conversation"] textarea{font-size:16px}html[data-lan-device="phone"] [data-slot="conversation"] button,html[data-lan-device="phone"] [data-slot="sidebar"] button{min-height:44px;touch-action:manipulation}html[data-lan-device="phone"] [data-slot="conversation.composer.bar"] button[aria-haspopup="menu"]{max-width:110px;height:30px;gap:2px;padding:0 8px;font-size:11.5px;line-height:18px;border-radius:999px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;flex:none}html[data-lan-device="phone"] [data-slot="conversation.input.model"] button[aria-haspopup="menu"] > span:first-child{max-width:64px;overflow:hidden;text-overflow:ellipsis;font-size:11.5px}html[data-lan-device="phone"] [data-slot="conversation.input.model"] button[aria-haspopup="menu"] > span:nth-child(2),html[data-lan-device="phone"] .Sh0Q9G_triggerLabel{display:none}html[data-lan-device="phone"] [data-slot="conversation.input.model"] [role="menu"]{position:fixed;right:max(8px,env(safe-area-inset-right));left:auto;top:96px;bottom:auto;width:min(240px,calc(100vw - 72px));max-height:min(360px,calc(100dvh - 150px));z-index:120}@media (max-width:820px){html:not([data-lan-device="desktop"]){font-size:16px}}</style>'
...
function injectHtml(html, kind, adminEntry) {
  ...
  const inject = '<script>' + UUID_POLYFILL + '</script>' +
    ...
    DEVICE_CSS + '<link rel="stylesheet" href="/pwa/app.css">' +
    pwaBoot()
