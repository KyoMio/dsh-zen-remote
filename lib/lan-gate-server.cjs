...
const COPY = {
  zh: {
    pairH1: '设备配对',
    pairSub: '在电脑上打开管理页生成配对码，输入后即可使用',
    pairCode: '配对码',
    pairName: '设备名（可选），如：我的手机',
    pairGo: '配对',
    pairAdmin: '管理页：本机浏览器访问',
    ...
    adminH1: '网关管理',
    adminSub: '配对码 + 设备令牌 · 经反代对外',
    running: '运行中',
    codeLabel: '配对码',
    codeEmpty: '尚未生成',
    codeNew: '生成配对码',
    codeNote: '配对码 10 分钟有效、一次性。在新设备的配对页输入即可完成配对。',
    codeGone: '尚未生成（或已使用/过期）',
    validUntil: '有效至 ',
    devLabel: '已配对设备',
    loading: '加载中…',
    noDevices: '暂无已配对设备',
    ...
    revoke: '吊销',
    revokeAll: '全部吊销',
    devNote: '访问方式：手机=紧凑排版+PWA，电脑=桌面布局，自动=按设备自适应。吊销立即生效并同时删除该设备的推送订阅。',
    statusErr: '无法读取状态',
    kindAuto: '自动',
    kindPhone: '手机',
    kindDesktop: '电脑',
    ...
    tapOpen: '打开 DSH 界面',
    upstreamToken: 'DSH 上游鉴权：token（自动交换）',
    upstreamNone: 'DSH 上游鉴权：无（0.1.1 直连模式）'
  },
  en: {
    pairH1: 'Device Pairing',
    pairSub: 'Open the admin page on your computer to generate a pairing code, then enter it',
    pairCode: 'Pairing code',
    pairName: 'Device name (optional), e.g. My phone',
    pairGo: 'Pair',
    pairAdmin: 'Admin page: open in a browser on this computer',
    ...
    adminSub: 'Pairing code + device token · exposed via reverse proxy',
    running: 'Running',
    codeLabel: 'Pairing code',
    codeEmpty: 'Not generated yet',
    codeNew: 'Generate pairing code',
    codeNote: 'The pairing code is valid for 10 minutes and can only be used once. Enter it on the new device’s pairing page to complete pairing.',
    codeGone: 'Not generated (or already used or expired)',
    validUntil: 'Valid until ',
    devLabel: 'Paired devices',
    loading: 'Loading…',
    ...
    devNote: 'Access mode: phone = compact layout + PWA, desktop = desktop layout, auto = adapt per device. Revoking takes effect immediately and also removes that device’s push subscription.',
    statusErr: 'Unable to read status',
    kindAuto: 'Auto',
    kindPhone: 'Phone',
    kindDesktop: 'Desktop',
    chipTitle: 'Device Pairing Admin',
    chipText: 'Pairing Admin',
    ...
    tapOpen: 'Open DSH',
    upstreamToken: 'Upstream auth: token (auto exchange)',
    upstreamNone: 'Upstream auth: none (0.1.1 direct mode)'
  }
}
...
function adminChip(lang) { var C = copyOf(lang); return '<a href="/lan-gate/admin" title="' + C.chipTitle + '" style="position:fixed;right:14px;bottom:14px;z-index:2147483000;font:12px system-ui;padding:6px 12px;border-radius:999px;background:rgba(22,26,34,.9);color:#9cc0ff;border:1px solid #2a2f3a;text-decoration:none;opacity:.8">⚙ ' + C.chipText + '</a>' }

function injectHtml(html, kind, adminEntry, lang) {
  ...
  if (adminEntry) {
    ...
    html = bodyClose >= 0 ? html.slice(0, bodyClose) + adminChip(lang) + html.slice(bodyClose) : html + adminChip(lang)
  }
  return html
}
...
function forwardRequest(req, res, client, device, isLocal) {
  ...
  var upstream = http.request({ host: TARGET_HOST, port: TARGET_PORT, method: req.method, path: req.url, headers: headers }, function (upRes) {
    ...
    if (UPSTREAM_TOKEN !== null && upRes.statusCode === 401 && isHtmlNavigation(req)) {
    ...
    if (isHtml) {
      ...
      upRes.on('end', function () {
        ...
        try {
          var html = injectHtml(Buffer.concat(chunks).toString('utf8'), kind, isLocal === true, langOf(req))
