...
function claimHandler(req, res, client) {
  ...
  readJsonBody(req, 4096, function (body) {
    ...
    var name = String(body.name || '').slice(0, 40).trim() || ('Device ' + id.slice(0, 4))
...
function pairingPage() {
  return gatePage('Device Pairing · DSH', '<div class="logo">DSH</div><h1>Device Pairing</h1><p class="sub">Open the admin page on your computer to generate a pairing code, then enter it</p>' +
    '<input id="code" maxlength="8" autocomplete="one-time-code" inputmode="text" placeholder="Pairing code">' +
    '<input id="name" maxlength="40" placeholder="Device name (optional), e.g. My phone">' +
    '<button class="btn" id="go">Pair</button><p class="bad" id="err"></p>' +
    '<p>Admin page: open in a browser on this computer<br>http://127.0.0.1:' + PROXY_PORT + '/lan-gate/admin</p>' +
    '<script>document.getElementById("go").addEventListener("click",function(){var b=this;b.disabled=true;fetch("/lan-gate/pair/claim",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({code:document.getElementById("code").value,name:document.getElementById("name").value})}).then(function(r){return r.json().then(function(j){return{s:r.status,j:j}})}).then(function(r){if(r.j&&r.j.ok){location.replace("/");return}b.disabled=false;var e=document.getElementById("err");e.textContent=r.s===429?"Too many attempts, please try again in 15 minutes":"Incorrect or expired pairing code"}).catch(function(){b.disabled=false;document.getElementById("err").textContent="Network error, please retry"})});</script>')
}
function rateLimitPage(ip) { return gatePage('Too Many Requests · DSH', '<div class="logo">DSH</div><h1>Too Many Requests</h1><p class="bad">Exceeded the limit of ' + RATE_LIMIT_PER_MIN + ' requests per minute. Please slow down.</p>') }

function adminPage() {
  return '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#0f1115"><title>DSH Gateway · Admin</title><style>*{box-sizing:border-box}html,body{margin:0;padding:0}body{min-height:100dvh;font-family:system-ui,-apple-system,"Segoe UI","PingFang SC","Microsoft YaHei",sans-serif;background:radial-gradient(1100px 700px at 50% -10%,#1b2233 0%,#0f1115 55%);color:#e6e8ec;padding:20px 16px 30px}.card{width:100%;max-width:560px;margin:0 auto;padding:28px 24px 24px;border:1px solid #2a2f3a;border-radius:20px;background:#161a22;box-shadow:0 20px 60px rgba(0,0,0,.45)}.head{display:flex;align-items:center;gap:12px;margin-bottom:18px}.logo{width:44px;height:44px;flex:none;border-radius:13px;background:linear-gradient(135deg,#4c8dff,#7a5cff);display:flex;align-items:center;justify-content:center;font-size:19px;font-weight:700;color:#fff}h1{font-size:19px;margin:0}.sub{font-size:13px;color:#9aa3b2;margin:2px 0 0}.badge{display:inline-flex;align-items:center;gap:6px;font-size:12px;border-radius:999px;padding:3px 10px;border:1px solid #2a2f3a;color:#9aa3b2;margin:0 0 14px}.badge b{width:8px;height:8px;border-radius:50%;background:#34d399;box-shadow:0 0 8px #34d399}.label{font-size:13px;font-weight:600;margin:18px 0 8px}.code{font-family:ui-monospace,Consolas,monospace;font-size:26px;letter-spacing:6px;color:#9cc0ff;background:#0b0e14;border:1px solid #232a37;border-radius:10px;padding:12px 16px;text-align:center;margin:6px 0}.item{padding:10px 0;border-bottom:1px solid #232a37}.item:last-child{border-bottom:none}.name{font-size:14px;font-weight:600}.muted{font-size:12px;color:#9aa3b2;margin:3px 0;word-break:break-all}.row{display:flex;align-items:center;gap:8px;margin-top:8px;flex-wrap:wrap}.btn{font-size:13px;border:1px solid #2f3748;background:transparent;color:#e6e8ec;border-radius:9px;padding:7px 14px;cursor:pointer}.btn:active{background:#222838}.btn-primary{border-color:#4c8dff;color:#9cc0ff;background:rgba(76,141,255,.08)}.btn-danger{border-color:#f0716f;color:#f0716f}.kind{font-size:12px;border:1px solid #2f3748;background:transparent;color:#9aa3b2;border-radius:999px;padding:5px 12px;cursor:pointer}.kind.on{border-color:#4c8dff;color:#9cc0ff;background:rgba(76,141,255,.08)}.empty{font-size:13px;color:#9aa3b2;text-align:center;padding:12px 0}.note{font-size:12px;color:#6b7280;line-height:1.7;margin:16px 0 0}.err{color:#f0716f;font-size:13px}</style></head><body><div class="card"><div class="head"><div class="logo">DSH</div><div><h1>Gateway Admin</h1><p class="sub">Pairing code + device token · exposed via reverse proxy</p></div></div><span class="badge"><b></b>Running · 127.0.0.1:' + PROXY_PORT + ' → DSH ' + TARGET_PORT + '</span>' +
    '<div class="label">Pairing code</div><div id="pair" class="empty">Not generated yet</div><div class="row"><button class="btn btn-primary" data-act="new-code">Generate pairing code</button></div><p class="note">The pairing code is valid for 10 minutes and can only be used once. Enter it on the new device’s pairing page to complete pairing.</p>' +
    '<div class="label">Paired devices</div><div id="devices" class="empty">Loading…</div>' +
    '<div class="row" style="margin-top:14px"><button class="btn btn-danger" data-act="revoke-all">Revoke all</button></div>' +
    '<p class="note">Access mode: phone = compact layout + PWA, desktop = desktop layout, auto = adapt per device. Revoking takes effect immediately and also removes that device’s push subscription.</p><div id="err"></div></div><script>' +
    'var KIND={auto:"Auto",phone:"Phone",desktop:"Desktop"};function post(body){return fetch("/lan-gate/action",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)}).then(function(){refresh()}).catch(function(){})}function esc(s){return String(s==null?"":s).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function fmt(ms){return ms?new Date(ms).toLocaleString():""}' +
    'function render(s){var pairEl=document.getElementById("pair");if(s.pairing&&s.pairing.code){pairEl.innerHTML="<div class=\\"code\\">"+esc(s.pairing.code)+"</div><div class=\\"muted\\">Valid until "+fmt(s.pairing.expiresAt)+"</div>";pairEl.className=""}else{pairEl.textContent="Not generated (or already used or expired)";pairEl.className="empty"}' +
    'var ds=s.devices||[],h="";if(!ds.length)h="<div class=\\"empty\\">No paired devices</div>";for(var i=0;i<ds.length;i++){var d=ds[i];var kb="";var ks=["auto","phone","desktop"];for(var j=0;j<ks.length;j++){var k=ks[j];kb+="<button class=\\"kind"+(d.kind===k?" on":"")+"\\" data-act=\\"set-kind\\" data-id=\\""+esc(d.id)+"\\" data-kind=\\""+k+"\\">"+KIND[k]+"</button>"}h+="<div class=\\"item\\"><div class=\\"name\\">"+esc(d.name)+(d.hasPush?" · 🔔":"")+"</div><div class=\\"muted\\">"+esc(d.ua||"")+"</div><div class=\\"muted\\">Paired "+fmt(d.createdAt)+" · last seen "+fmt(d.lastSeen)+"</div><div class=\\"row\\">"+kb+"<button class=\\"btn btn-danger\\" data-act=\\"revoke\\" data-id=\\""+esc(d.id)+"\\">Revoke</button></div></div>"}document.getElementById("devices").innerHTML=h}' +
    'function refresh(){fetch("/lan-gate/status").then(function(r){return r.json()}).then(render).catch(function(){var e=document.getElementById("err");if(e)e.innerHTML="<div class=\\"err\\">Unable to read status</div>"})}' +
...
const ADMIN_CHIP = '<a href="/lan-gate/admin" title="Device Pairing Admin" style="position:fixed;right:14px;bottom:14px;z-index:2147483000;font:12px system-ui;padding:6px 12px;border-radius:999px;background:rgba(22,26,34,.9);color:#9cc0ff;border:1px solid #2a2f3a;text-decoration:none;opacity:.8">⚙ Pairing Admin</a>'
...
function pushSendHandler(req, res) {
  ...
  readJsonBody(req, 16384, function (body) {
    ...
    var payload = JSON.stringify({ title: String(body.title || 'DSH task finished').slice(0, 80), body: String(body.body || '').slice(0, 120), tag: String(body.tag || 'dsh-agent-done'), data: { url: '/' } })
