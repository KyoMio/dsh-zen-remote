...
function MobileHomeChips({ t, sessionId, downloadSessionLog, onCustomize }) {
    ...
    return ((0, jsx_runtime_1.jsxs)("div", { "data-mobile-nav": "chip-row", children: [visible.map((def) => ((0, jsx_runtime_1.jsxs)("button", { type: "button", "data-mobile-nav": "chip", onClick: () => activate(def, sessionId, downloadSessionLog), children: [(0, jsx_runtime_1.jsx)(def.Icon, { size: 16 }), (0, jsx_runtime_1.jsx)("span", { children: t(def.label) })] }, def.id))), (0, jsx_runtime_1.jsx)("button", { type: "button", "data-mobile-nav": "chip-more", "aria-label": t('chipCustomize'), title: t('chipCustomize'), onClick: onCustomize, children: (0, jsx_runtime_1.jsx)(dsh_client_ui_primitives_1.IconEllipsisOutline16, { size: 16 }) })] }));
}
/**
 ...
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.PROMPT_IMAGE_TYPES = exports.UPLOAD_ROUTE = void 0;
exports.promptImageType = promptImageType;
exports.buildImageParts = buildImageParts;
 ...
 */
exports.PROMPT_IMAGE_TYPES = ['image/png', 'image/jpeg', 'image/webp', 'image/gif'];
/**
 ...
 */
function promptImageType(mediaType) {
    const value = mediaType.split(';', 1)[0]?.trim().toLowerCase() ?? '';
    return exports.PROMPT_IMAGE_TYPES.find((known) => known === value);
}
/**
 ...
 */
function buildImageParts(images, text) {
    const parts = [];
    if (text.trim() !== '')
 ...
 */
function MobileAttachButton({ t, sessionId, useInput, inputActions, promptImages }) {
    ...
    const pick = async (event) => {
        ...
        try {
            for (const file of files) {
                const mediaType = (0, attach_upload_ts_1.promptImageType)(file.type);
                if (mediaType !== undefined) {
                    images.push({ name: file.name, mediaType, base64: (0, attach_upload_ts_1.bytesToBase64)(new Uint8Array(await file.arrayBuffer())) });
                    continue;
                }
                written = (0, attach_upload_ts_1.draftWithMention)(base(), await uploadFile(sessionId, file));
                inputActions.setDraft(written);
            }
            if (images.length > 0) {
                const text = base();
                const sent = await promptImages(sessionId, (0, attach_upload_ts_1.buildImageParts)(images, text));
                if (sent === undefined || !sent.ok)
                    throw new Error(t('attachFailed'));
...
function apply(ctx) {
    ...
    ctx.slots.inject('conversation.input.left', () => ctx.slots.register({
        ...
        locale: locales_ts_1.NS,
        inject: (_sessionId) => ({
            promptImages: (id, content) => ctx.sessions.binding(id)?.session.prompt(content, 'queue'),
        }),
