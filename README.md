...
- **手机端附件上传**:composer 最左的回形针按钮打开**手机本地**的选择器(不设 accept,iOS 弹出 相册 / 拍照 / 选取文件 全选单;官方那套是在跑 DSH 的电脑上弹窗,手机远程用不了)。图片和文件一视同仁:都上传到会话工作目录的 `.dsh-uploads/`,并把 `@.dsh-uploads/文件名` 追加到输入框,输入卡上方出现预览 chip(图片显示缩略图,其余显示图标+文件名,带 × 移除),由你按官方发送键决定何时发出。模型读图靠 `@路径` + 你自己装的读图工具链。**这会在插件的 host 半区注册一条 HTTP 路由** `POST /_dsh/mobile-nav/upload`:只接受同源请求、文件名清洗后只取叶子名(空白折成 `_`)、写入路径校验不得越出工作目录、单个文件默认上限 20MB(在 profile 的 `dsh-mobile-nav` 插件行加 `maxUploadBytes` 可调)。远程访问时它和其它请求一样在 dsh-mobile-pwa 网关的配对墙之后。仅 <768px 可见。
