...
| --- | --- | --- | --- |
| ![会话列表主屏](assets/hero.png) | ![会话页](assets/session.png) | ![会话信息卡](assets/info.png) | ![composer 权限 sheet](assets/sheet.png) |
...
- 会话列表上方一条可横滑的插件入口 chips（文件浏览、用量、Session log 等，按你实际装的插件动态出现），行尾「···」打开显隐自定义单，选择保存在本地，刷新后还在；
...
安全上的几句话：这个功能给插件的 host 半区新增了一条 HTTP 路由（`POST /_dsh/mobile-nav/upload`），只接受同源请求，文件名清洗后只取叶子名，写入路径校验不允许越出工作目录，单文件默认上限 20MB（可在 profile 里的插件行用 `maxUploadBytes` 调整）。远程访问时，这条路由和其它请求一样在 dsh-mobile-pwa 网关的配对墙之后。
...
- 主屏插件入口 chips：任务看板/SSH/文件浏览/用量等按已装插件动态出现，显隐可自定义并本地持久化；
...
- PWA 冷启动首帧安全区未就位（首帧 `viewport-fit=cover` 改由 dsh-mobile-pwa 网关注入，不再等插件 JS 迟到补）；
- 会话行三点菜单弹出时抽屉/列表状态错乱、workbench 关闭按钮在桌面端泄漏等一系列实机反馈热修。

**内部**

- 作用域与 dsh-mobile-pwa 理清分工：本插件管排版与交互，dsh-mobile-pwa 只管通道（认证/推送/PWA 壳），两边 CSS 不再重叠；
- 新增 host 半区（`src/index.ts`）：从纯 client 插件变为带一条上传路由的插件；
- 大量实机反馈驱动的迭代热修（S1–S8 各切片），完整过程见 `docs/superpowers/` 下的设计与实现记录。
