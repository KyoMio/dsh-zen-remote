...
@media (max-width: 767px) {
  /* [viewport fix] The desktop app pins the main conversation column to
     min-width:440px (.dsh-sc-column.dsh-sc-column-main). On phones narrower
     than 440 CSS px — 360-439 covers most Android devices, e.g. Mate 70 Pro
     at 376px — the whole page overflows by up to 64px, and the document-level
     scroll lock (keyboard avoidance) turns that overflow into clipped text:
     every line loses its trailing characters and the header actions disappear
     off-screen. Verified at 376×770 dpr 3.5 via CDP with the same viewport:
     out-of-viewport elements 506 → 7, prose fully visible. Harmless on
     profiles whose base styles carry no such pin. */
  .dsh-sc-column.dsh-sc-column-main {
